<x-admin-layout>
    ...
    <header class="header_post_edit">
        ...
        <span class="leave-compare" onclick="leaveCompare();">Lower the comparison</span>
        <span class="info">History mail</span>
        <div class="profile">
    ...
    <div class="post__history">
        <div class="compact-history-list">
            <div class="extend-history" onclick="extend_history();">Show a compact history</div>
            <div id="history_list" style="height: 0px; visibility: hidden;">
                ...
                @if ($autoSave)
                    <div class="date">
                        Changes with {{ \Carbon\Carbon::parse($lastDate)->translatedFormat('d F, Y') }}
                ...
                @foreach ($historyPosts as $historyPost)
                    ...
                    @if ($lastDate != $postDate && $historyPost->additional_info != 2)
                        ...
                        <div class="date">
                            <div class="line-v"></div>
                            Changes with {{ \Carbon\Carbon::parse($postDate)->translatedFormat('d F, Y') }}
                            <div class="line-v"></div>
                        </div>
                        ...
                        <div class="body">
                            ...
                            @if ($historyPost->additional_info !== 2)
                                <span class="actions{{ (int) $history_id === $historyPost->id ? '' : ' hidden' }}">
                                    <span onClick="revert({{ $id }}, {{ $historyPost->id }});">Przywróć <i
                                            class="fa-solid fa-clock-rotate-left"></i></span>
                                </span>
                            @endif
                            <span onclick="compare(event, {{ $historyPost->id }});"
                                class="compare{{ (int) $history_id === $historyPost->id ? ' hidden' : '' }}">Compare <i
                                    class="fa-solid fa-right-left"></i></span>
                        </div>
                    </div>
                @endforeach
        ...
        <aside class="post__preview" id="first_post">
            ...
            <div class="post_options">
                ...
                <div class="published">
                    <p>Set visible to public</p>
                    <label class="switch">
                        <input type="checkbox" name="is_published"
                            {{ isset($post) ? ($post->is_published ? 'checked' : '') : 'checked' }} disabled>
